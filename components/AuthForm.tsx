...
export const AuthForm: React.FC<AuthFormProps> = ({ view, onNavigate, onAuthSuccess }) => {
  ...
  const [activeTab, setActiveTab] = useState<'LOGIN' | 'SIGNUP'>(view);

  // Sync internal tab state with parent view prop if needed, 
  // but usually user clicks tabs locally.
  const handleTabChange = (tab: 'LOGIN' | 'SIGNUP') => {
    setActiveTab(tab);
    // Optional: notify parent if we want URL/History updates, 
    // but for this SPA, local state switch is smoother visually
    // onNavigate(tab); 
    setError('');
  };
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center items-center px-6 sm:px-12 xl:px-24 bg-black relative">
        ...
        <div className="w-full max-w-sm mt-12 lg:mt-0">
            ...
            <div className="mt-8 space-y-4 text-center">
                 ...
                 <button className="text-sm font-bold text-slate-400 hover:text-white transition-colors block w-full">Forgot password?</button>
